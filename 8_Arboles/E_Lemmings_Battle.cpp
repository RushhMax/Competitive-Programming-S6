...
#define _vec(x) {cout << #x << " = "; for(auto & i : x){cout << i << " ";} cout << endl;} //Print entire vector
...
#define j(n) for(int j=0;j<n;j++) //From 0 to n-1
...
void solve(){
    ...
    while(!blue.empty() && !green.empty()){
        ...
        _vec(green);
        _vec(blue);
        auto greenPL = green.rbegin();
        auto bluePL = blue.rbegin();

        vector<int> difG, difB;
        
        for(int i=0; i < b && greenPL != green.rend() && bluePL != blue.rend() ; i++){   
            cout<<"Comparing: "<<*greenPL<<" vs "<<*bluePL<<endl;
            if(*greenPL != *bluePL){
                int dif = (*greenPL > *bluePL) ? *greenPL - *bluePL : *bluePL - *greenPL;
                (*greenPL > *bluePL) ? difG.pb(dif) : difB.pb(dif);
                cout<<((*greenPL > *bluePL)? *greenPL : *bluePL)<<endl;
            }
        ...
        f(i, difG.size()){
            cout<<" INSERYANDO "<<difG[i]<<endl;
            green.insert(difG[i]);
        }
        f(i, difB.size()){
            cout<<" INSERYANDO "<<difB[i]<<endl;
            blue.insert(difB[i]);
        }
    }
...
int main(){
    ...
    cin.tie(nullptr); // Para optimizar la entrada/salida
