...
struct Recipe{
    ...
    Recipe(string name, int n_ingredients){
        this->name = name;
        this->n_ingredients = n_ingredients;
    }

    int price_TOTAL(map<string, int> prices){
        int total = 0;
        for(auto& i : ingredients){
            total += i.second * prices[i.first];
        }
        p_total = total;
        return total;
    }
...
void solve(){
    ...
    // Ingredientes
    f(i,m){
        string ing; cin>>ing; cin.ignore();
        int price; cin>>price; cin.ignore();
        ingredients[ing] = price;
    }

    // Recetas
    map<int, set<string>> recipes;
    f(i,n){
        string name; getline(cin,name);
        int n_ingred; cin>>n_ingred; cin.ignore();

        //Recipe r(name, n_ingred);
        int acum = 0;
        f(j,n_ingred){
            string ing; cin>>ing; cin.ignore();
            int cant; cin>>cant; cin.ignore();
            acum += cant * ingredients[ing];
            //r.ingredients[ing] = cant;
        }

        if(acum <= b){
            recipes[acum].insert(name);
        }
    }
