...
int lengthPath(int i, int j, vector<vector<char>> &grid, vector<vector<int>> &dp, int H, int W){
    ...
    f(k,8){
        ...
        if(ni >= 0 && nj >= 0 && ni < H && nj < W){
            if(grid[i][j]+1 == grid[ni][nj] ){
                maxLength = max(maxLength, 1 + lengthPath(ni,nj, grid, dp, H,W));
            }
        }
    }
...
int main(){
    ...
    while(cin>>H>>W && H!=0 && W!=0){
        ...
        int maxPath = -1;
        f(i,H) f(j,W) maxPath = max(maxPath, lengthPath(i,j, grid, dp, H,W));

        cout<<"Case "<<tc++<<": "<<maxPath<<endl;
    }
