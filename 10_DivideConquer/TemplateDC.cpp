...
int combinar(int izq, int der) {
    // LOGICA
    return max(izq, der); 
}

int divideYConquistar(vector<int>& datos, int inicio, int fin) {
    if (inicio == fin) {
        return datos[inicio]; // O el valor que quieras retornar
    }

    int medio = inicio + (fin - inicio) / 2;

    int parteIzquierda = divideYConquistar(datos, inicio, medio); 
    int parteDerecha = divideYConquistar(datos, medio + 1, fin);  

    return combinar(parteIzquierda, parteDerecha);
}

void solve(){
    vector<int> datos = {3, 5, 2, 9, 4, 7, 1, 6};

    int resultado = divideYConquistar(datos, 0, datos.size() - 1);
